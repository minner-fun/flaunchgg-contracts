...
/// @dev This library is intended for `forge test` environments. There may be gotchas when using salts in `forge script`
/// or `forge create`
library HookMiner {
    ...
    function computeAddress(
        ...
        return
            address(uint160(uint(keccak256(abi.encodePacked(bytes1(0xFF), deployer, salt, keccak256(creationCode))))));
    }
}
