...
contract FeeEscrowTest is FlaunchTest {
    ...
    function test_AllocateFees(address _recipient, uint _amount) public {
    ...
    function test_AllocateFeesWithZeroAmount(
        address _recipient
    ) public {
        // Validate the recipient address
        vm.assume(_recipient != address(0));
    ...
    function test_WithdrawFees(uint _amount, bool _unwrap) public {
        ...
        if (_unwrap) {
            ...
            assertEq(
                IERC20(address(flETH)).balanceOf(_recipient),
                _amount,
                'Recipient did not receive correct amount of flETH'
            );
        }
    }

    function test_WithdrawFeesWithZeroBalance(
        bool _unwrap
    ) public {
        // Create valid addresses for testing
        address payable _sender = payable(makeAddr('_sender'));
    ...
    function test_WithdrawFeesMultipleTimes(
        uint _amount
    ) public {
        // Create valid addresses for testing
        address payable _sender = payable(makeAddr('_sender'));
        ...
        assertEq(
            IERC20(address(flETH)).balanceOf(_recipient), _amount * 2, 'Recipient did not receive correct total amount'
        );
    }
