...
library ProxyCheck {
    ...
    function getImplementation(
        ...
        if (
            code.length == 45 && uint8(code[0]) == 0x36 && uint8(code[1]) == 0x3d && uint8(code[2]) == 0x3d
                && uint8(code[3]) == 0x37 && uint8(code[4]) == 0x3d && uint8(code[5]) == 0x3d && uint8(code[6]) == 0x3d
                && uint8(code[7]) == 0x36 && uint8(code[8]) == 0x3d && uint8(code[9]) == 0x73
        ) {
            assembly {
