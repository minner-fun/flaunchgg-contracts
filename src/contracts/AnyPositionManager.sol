...
/**
 * The AnyPositionManager is a Uniswap V4 hook enables any ERC20 token to have BidWall and distribute fees with the
 * community.
 *
...
contract AnyPositionManager is BaseHook, FeeDistributor, InternalSwapPool, StoreKeys {
    ...
    function flaunch(
        ...
        if (!approvedMemecoinCreator[msg.sender]) {
            revert CallerIsNotApprovedCreator();
        }

        // Check if the memecoin has already been flaunched
        if (flaunchContract.tokenId(_params.memecoin) != 0) {
            revert AlreadyFlaunched();
        }
    ...
    function beforeSwap(
        ...
        bidWall.checkStalePosition({_poolKey: _key, _currentTick: _beforeSwapTick, _nativeIsZero: nativeIsZero});
